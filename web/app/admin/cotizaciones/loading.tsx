      ...
      <div className="overflow-x-auto rounded-lg border border-white/10">
        <table className="w-full">
          <thead className="bg-navy text-brand-gray text-xs uppercase tracking-widest">
            <tr>
              {['No.', 'Cliente', 'Concepto', 'Total', 'Estado', 'Fecha', ''].map((h, i) => (
                <th key={i} className="px-4 py-3 text-left">
                  <div className="h-3 bg-white/10 rounded animate-pulse" style={{ width: h ? '4rem' : '2rem' }} />
                </th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {Array.from({ length: 5 }).map((_, i) => (
              <tr key={i} className="bg-navy-2">
                ...
                <td className="px-4 py-3"><div className="h-4 w-40 bg-white/10 rounded animate-pulse" /></td>
